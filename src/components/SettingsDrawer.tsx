    ...
    <>
      <dialog ref={ref} className="drawer" aria-labelledby="drawerTitle" onClick={handleBackdropClick}>
        ...
        <div className="drawer-body">
          ...
          <section className="setting-group">
            <h3><Layers />外观</h3>
            <div className="field"><span>高对比模式<small>增强课程卡与文字边界</small></span><button className="switch" type="button" role="switch" aria-label="高对比模式" aria-checked={reduceTransparency} onClick={onToggleTransparency}><span /></button></div>
            <div className="field"><span>暂停动态效果<small>减少动画与过渡</small></span><button className="switch" type="button" role="switch" aria-label="暂停动态效果" aria-checked={motionPaused} onClick={onToggleMotion}><span /></button></div>
          </section>
          <section className="setting-group">
            <h3><Bell />课前提醒<span className="group-tag">{reminderEnabled ? '已开启' : '未开启'}</span></h3>
            <div className="field"><span>浏览器通知<small>每节课前 15 分钟提醒</small></span><button className="switch" type="button" role="switch" aria-label="浏览器通知" aria-checked={reminderEnabled} onClick={onToggleReminder}><span /></button></div>
