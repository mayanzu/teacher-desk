...
describe('buildSluImportPayload', () => {
  it('builds a teacher profile from the authenticated schedule HTML', () => {
    const html = `<table>
      ...
      <tr><td rowspan="2">上午</td><td>1(08:10-08:55)</td><td>数据结构与算法 [2-17]周 1-2节 30 H楼505 示例一班</td><td></td><td></td></tr>
      <tr><td>2(09:00-09:45)</td><td></td><td>离散数学 [2-17]周 1-2节 30 D楼208 示例一班</td><td></td></tr>
    </table>`;
    ...
    expect(payload.meta.teacher).toBe('示例教师');
  ...
  it('extracts a clean teacher name from weeklesson HTML and keeps last-parity variants', () => {
    ...
    expect(payload.courses[0]).toMatchObject({ name: '示例课程C实验', day: 2, slot: '7-8', weeks: '7-17', parity: 'odd' });
  });
});
